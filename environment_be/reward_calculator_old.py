    ...
    def calculate_reward(self, solver_status, termination_condition, new_mix, demand_profile, cur_epoch,service_level_performance):
        ...
        if not cond_b:
            ...
            for key,value in demand_profile.items():
                ...
                # for same unmet service level, higher penalty for higher ranked customers
                instance_reward = (1-unmet_service_level) * (customer_rank_score/self.sum_of_customer_ranks)
            ...
            for customer in self.customer_rank:
                service_level_performance[customer] = 0.9 * service_level_performance[customer] + 0.1 * np.mean(epoch_end_customer_service_level_dict[customer])
