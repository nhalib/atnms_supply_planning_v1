...
class dumb_agent():
    ...
    def _simulator(self):
        ...
        for timestep in range(self.max_time_steps):
            ...
            action = np.random.choice(self.action_count) # uniformly random action
        ...
        pd.DataFrame(self.reward_plot_df).to_excel("_data/mean_reward_da_ur.xlsx")

        pd.DataFrame(self.viz_df).to_excel("_data/viz_df_da_ur_agent.xlsx")

        [lt_performance_df] = self.environment._environment_lt_perf_data()
        pd.DataFrame(lt_performance_df).to_excel("_data/lt_perf_da_ur.xlsx")
